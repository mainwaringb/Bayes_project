...
These expectations were codified using loose rules of thumb for both the constituency and individual level. Priors for $\Gamma$ (at the constituency level) assumed that a shift from the 25th and 75th percentiles of the IV would correspond with a 1-point shift in the DV. [^7] Priors on $\beta$ (at the individual level) used a similar rule of thumb: each variable mwould have an approximately 1-point effect. [^8] Standard deviations on the priors were intentionally broad, to reflect that they were no more than educated guesses. 
...
[^12]: It is logically necessary that people in an AfD-supporting constituency are more likely to be AfD supports. Constituenecy-level variables that are correlated with AfD vote share would therefore *appear* correlated with individual-level AfD support, even if the correlation is spurious.

[^13]: Informal experimentation with various sets of variables is often a part of model-building. Computationally intensive models impose discipline, by contraining experimentation. This can be helpful, because it avoids overfitting and demands good theory. However, it might require a stronger theory than I had when pursuing this project.
